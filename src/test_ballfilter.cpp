...
int main(int argc, char** argv)
{
  if (argc <3)
    {
      PCL_ERROR("Usage: %s <input.pcd> <reference.pcd>\n", argv[0]);
      return -1;
    }
  ...
  if(pcl::io::loadPCDFile<pcl::PointXYZ> (argv[2], *ref)==-1)
    {
      PCL_ERROR("Couldn't read file %s\n", argv[2]);
      return -1;
    }

  // This is a dumb way to locate a good initial guess and shake off some outliers
  BallSelector bf(0.128, 0.03);
  bf.SetInputCloud(cloud);
  if (bf.ComputeScore() < 0.0)
    {
  ...
  viewer.addPointCloud<pcl::PointXYZRGB> (ccl, rgb, "seg cloud");
  viewer.addPointCloud<pcl::PointXYZ> (ref, "ref cloud");
  viewer.setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "seg cloud");
